# This train.py script is modified from the torchfm library: https://github.com/rixwew/pytorch-fm/blob/master/examples/main.py and is thus not the contribution of Terrorch.
import torch
import tqdm
...
def main(dataset_name,
    ...
    # test_data_loader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=0)
    test_data_loader = DataLoader(test_dataset, batch_size=batch_size, num_workers=0)
    model = get_model(model_name, dataset).to(device)
    criterion = torch.nn.BCELoss()
    optimizer = torch.optim.Adam(params=model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    early_stopper = EarlyStopper(num_trials=3, save_path=f'{save_dir}/{model_name}_{dataset_name}.pt')
